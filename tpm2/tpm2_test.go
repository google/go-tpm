...
func TestCombinedNvTest(t *testing.T) {
	...
	handle := Handle((OrdTPM_HT_NV_INDEX << OrdHR_SHIFT) + 1000)
	owner := Handle(OrdTPM_RH_OWNER)
	UndefineSpace(rw, owner, handle)
	dataSize := uint16(8)
	offset := uint16(0)
	...
	if err != nil {
		t.Fatalf("DefineSpace failed: %v", err)
	}
	defer UndefineSpace(rw, owner, handle)
	...
	if err != nil {
		t.Fatalf("IncrementNv failed: %v", err)
	}
	c1, err := ReadNv(rw, handle, authString, offset, dataSize)
	if err != nil {
		t.Fatalf("ReadNv failed: %v", err)
	}
	err = IncrementNv(rw, handle, authString)
	if err != nil {
		t.Fatalf("IncrementNv failed: %v", err)
