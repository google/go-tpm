...
func AESEncryption(keySize tpm.KeyBits, dir parameterEncryptiontpm2ion) AuthOption {
	return func(o *sessionOptions) {
...
func getEncryptedSaltECC(nameAlg tpmi.AlgHash, parms *tpms.ECCParms, pub *tpms.ECCPoint) (*tpm2b.EncryptedSecret, []byte, error) {
	...
	salt := helpers.KDFeHash(ha, z, "SECRET", ephPubX.Bytes(), pub.X.Buffer, ha.Size()*8)
...
func (s *hmacSession) Init(t transport.TPM) error {
	...
	if s.bindHandle != tpm.RHNull || len(salt) != 0 {
		...
		s.sessionKey = helpers.KDFaHash(ha, authSalt, "ATH", s.nonceTPM.Buffer, s.nonceCaller.Buffer, ha.Size()*8)
	}
	return nil
}
...
func (s *hmacSession) Encrypt(parameter []byte) error {
	...
	keyIV := helpers.KDFaHash(ha, sessionValue, "CFB", s.nonceCaller.Buffer, s.nonceTPM.Buffer, keyIVBytes*8)
...
func (s *hmacSession) Decrypt(parameter []byte) error {
	...
	keyIV := helpers.KDFaHash(ha, sessionValue, "CFB", s.nonceTPM.Buffer, s.nonceCaller.Buffer, keyIVBytes*8)
	key, err := aes.NewCipher(keyIV[:keyBytes])
	if err != nil {
		return err
	}
